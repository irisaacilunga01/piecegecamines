...
import { deleteDetailCommande } from "@/lib/actions";
...
export const columns: ColumnDef<Payment>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="capitalize">{row.getValue("quantiteafournir")}</div>
    ),
  },
  {
    accessorKey: "quantitedemandee",
    header: ({ column }) => {
      return (
        <Button
          variant="ghost"
    ...
    cell: ({ row }) => (
      <div className="capitalize">{row.getValue("quantitedemandee")}</div>
    ),
  },
  {
    accessorKey: "bo",
    header: "Bo",
    cell: ({ row }) => (
      <div className="capitalize">{row.getValue("bo")}</div>
    ),
  },
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="flex gap-2 justify-center items-center">
                ...
                <p>Ajouter un détail pour la commande</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Link
                  className="flex gap-3"
                  href={`/dashboard/detailcommande/${row.original.numrecquisition}`}
                >
              ...
              <TooltipContent className="flex gap-2 justify-center items-center">
                ...
                <p>modifier le détail de la commande N° {row.original.numrecquisition}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
          ...
          <Dialog>
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent className="flex gap-2 justify-center items-center">
                  ...
                  <p>
                    supprimer le détail de la commande N° {row.original.numrecquisition}
                  </p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Etes-vous sur de supprimer définitivement le détail de la commande N° {row.original.numrecquisition} ?
                </DialogDescription>
              </DialogHeader>
              ...
              <DialogFooter className="justify-end">
                ...
                <Button
                  ...
                  onClick={async () => {
                    try {
                      await deleteDetailCommande(
                        Number(row.original.numrecquisition),Number(row.original.numarticle)
                      );
                      toast({
                        title: "Supprimer",
                        description: `le détail de la commande du magasin N° ${row.original.numrecquisition} a été supprimer avec succès`,
                        className: "bg-green-700 text-white",
                      });
                    } catch (error) {
                      toast({
                        title: "Erreur supprimer",
                        description: `Erreur lors de la suppression le détail de la commande`,
  ...
  return (
    <div className="w-full px-4 pt-4">
      <div className="flex items-center pb-4">
        <Input
          placeholder="Rechercher par nom pièce..."
          value={
